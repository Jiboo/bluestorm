...
//#define assertex(CODE, MESSAGE)  try { CODE; } catch(Exception e) { Log.e("bluetoothtest", MESSAGE); }
...
public class Nxt implements INxt {
    ...
    public Nxt() throws IOException {
        ...
        for (BluetoothDevice tmp : adapter.getBondedDevices()) {
            if (tmp.getName().equals("NXT")) {
                dev    = tmp;
                paired = true;
                Log.d("bluetoothtest", "NXT trouvé: " + tmp.getAddress());
            }
        }
    }
    ...
    public boolean connect() throws IOException {
        ...
        Log.d("bluetoothtest", "Connexion...");

        try {
            sock.connect();
        } catch (Exception e) {
            Log.e("bluetoothtest", "connexion impossible");
    ...
    private void send(byte[] ba) throws IOException {
        ...
        Log.d("bluetoothtest", "Trame envoyée: " + sb.toString());
    }

    private byte[] read() throws IOException {
        byte[] size_header = new byte[2];

        if (sock.getInputStream().read(size_header) == -1) {
            throw new IOException("Erreur lors de la lecture de l'entete");
        }

        int    size = size_header[0] + (size_header[1] << 8);
        byte[] ba   = new byte[size];

        if (sock.getInputStream().read(ba) == -1) {
            throw new IOException("Erreur lors de la lecture de la trame");
        }
        ...
        Log.d("bluetoothtest", "Trame lu: " + sb.toString());
    ...
    private void keepAlive() throws IOException {
        byte[] ba = {
            (byte) 0x02, (byte) 0x00, (byte) 0x80,    // Pas de réponse
            (byte) 0x0D,                              // KeepAlive
        };
    ...
    private void setOutputState(byte port, byte power, byte output_mode, byte regul_mode, byte turn_ratio,
                                byte run_state, long limit)
            throws IOException {
        byte[] ba = {
            (byte) 0x0c, (byte) 0x00, (byte) 0x80,    // Pas de réponse
            (byte) 0x04,                              // SetOutputState
            port, power, output_mode, regul_mode, turn_ratio, run_state, (byte) (limit & 0xFF),
            (byte) (limit >> 8 & 0xFF), (byte) (limit >> 16 & 0xFF), (byte) (limit >> 24 & 0xFF),
        };
    ...
    private void setInputMode(byte port, byte sensor_type, byte sensor_mode) throws IOException {
        byte[] ba = {
            (byte) 0x05, (byte) 0x00, (byte) 0x80,    // Pas de réponse
            (byte) 0x05,                              // SetInputMode
            port, sensor_type, sensor_mode
        };
    ...
    private void getInputValues(byte port) throws IOException {
        byte[] ba = { (byte) 0x03, (byte) 0x00, (byte) 0x00,    // Réponse
    ...
    public void setSpeed(byte pLeftSpeed, byte pRightSpeed) throws IOException {
        this.setOutputState(PORT_A, (byte) (pRightSpeed), OUTPUT_MODE_MOTORON, REGUL_MODE_IDLE, (byte) (0),
                            RUN_STATE_RUNNING, 0L);
        this.setOutputState(PORT_B, (byte) (pLeftSpeed), OUTPUT_MODE_MOTORON, REGUL_MODE_IDLE, (byte) (0),
                            RUN_STATE_RUNNING, 0L);
    }
    ...
    public void openClaw() throws IOException, InterruptedException {
        this.setOutputState(PORT_C, (byte) (STATIC_SPEED), OUTPUT_MODE_MOTORON, REGUL_MODE_IDLE, (byte) (0),
                            RUN_STATE_RUNNING, 20L);
        Thread.sleep(500);
    ...
    public void closeClaw() throws IOException, InterruptedException {
        this.setOutputState(PORT_C, (byte) (-(STATIC_SPEED / 1.5)), OUTPUT_MODE_MOTORON, REGUL_MODE_IDLE, (byte) (0),
                            RUN_STATE_RUNNING, 20L);
        Thread.sleep(1000);
        this.setOutputState(PORT_C, (byte) (0), OUTPUT_MODE_BRAKE, REGUL_MODE_IDLE, (byte) (0), RUN_STATE_IDLE, 0L);
    }
    ...
    public double getBatteryLevel() throws IOException {
        byte[] ba = {
            (byte) 0x02, (byte) 0x00, (byte) 0x00,    // Réponse
    ...
    public void emitTone(int pFreq, int pDur) throws IOException {
        byte[] ba = {
            (byte) 0x06, (byte) 0x00, (byte) 0x80,                                // Pas de réponse
            (byte) 0x03,                                                          // Emit tone
            (byte) (pFreq & (short) 0xFF), (byte) (pFreq >> 8 & (short) 0xFF),    // Freq (UWORD)
            (byte) (pDur & (short) 0xFF),
            (byte) (pDur >> 8 & (short) 0xFF)                                     // Dur en ms (UWORD)
        };
